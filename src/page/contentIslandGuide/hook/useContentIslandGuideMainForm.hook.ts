...
const initMapForm = (passData) => {
    return {
        islandGuideTypeId: passData?.type?.id || '',
        islandGuideAreaId: passData?.area?.id || '',
        name: passData.name || '',
...
const useContentIslandGuideMainFormHook = ({
    isEdit = false,
}: {
    isEdit?: boolean
}) => {
    ...
    const _handleSubmit = () => {
        ...
        return __handleSubmit({
            apiCall: () =>
                isEdit
                    ? apiIslandGuide.updateWithData(id, form)
                    : apiIslandGuide.addWithData(form),
            setIsLoading,
...
export default useContentIslandGuideMainFormHook
