...
const useIslandGuideAreaMainForm = ({
    isEdit = false,
}: {
    isEdit?: boolean
}) => {
    ...
    const dataDetail = useDetailFormRequestHook({
        urlAPI: () => apiIslandGuideArea.detail(id),
        formRequest,
        setFormRequest,
    ...
    const _handleSubmit = () => {
        return __handleSubmit({
            apiCall: () =>
                isEdit
                    ? apiIslandGuideArea.updateWithData(id, formRequest)
                    : apiIslandGuideArea.addWithData(formRequest),
            setIsLoading,
...
export default useIslandGuideAreaMainForm
