...
const useIslandGuideTypeMainForm = ({
    isEdit = false,
}: {
    isEdit?: boolean
}) => {
    ...
    const dataDetail = useDetailFormRequestHook({
        urlAPI: () => apiIslandGuideType.detail(id),
        formRequest,
        setFormRequest,
    ...
    const _handleSubmit = () => {
        return __handleSubmit({
            apiCall: () =>
                isEdit
                    ? apiIslandGuideType.updateWithData(id, formRequest)
                    : apiIslandGuideType.addWithData(formRequest),
            setIsLoading,
...
export default useIslandGuideTypeMainForm
