...
import useBlogStore from '@/store/useBlog.store.ts'
...
const shapeDataList = (passData: Record<string, any> = {}) => ({
    ...passData,
    value: passData.id,
    label: passData.title,
})

const SelectOptionBlog = (props: SelectOptionGeneralProps & ListDataProps) => {
    const ctx = useHookContextForm()

    const { __list, __handlePushDataStore } = useBlogStore({
        isFormatList: false,
    })
    ...
    const myId = id || 'select-blog' + name + useId()
            ...
            {dataList?.length ? (
                <>
                    <p className="fs-12 mb-2 fw-600">
                        Total Properties : {dataList.length}
                    </p>
                        ...
                        {dataList.map((blog, index) => {
                            return (
                                <div
                                    ...
                                    <div className="w-100">{blog.title}</div>
                                    <BtnCircleRemove
                                        className="ms-auto"
                                        actions={{
                                            remove: () =>
                                                dataActions.remove(blog),
                                        }}
                                    />
                                </div>
                            )
