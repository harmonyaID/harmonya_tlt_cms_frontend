...
import useAmenitiesCategoryStore from '@/store/usePropertyAmenitiesCategory.store.ts'

const SelectOptionPropertyAmenitiesCategory = (
    props: SelectOptionGeneralProps,
) => {
    const ctx = useHookContextForm()

    const { __list, __isLoading } = useAmenitiesCategoryStore({
        isFormatList: false,
    })
    ...
    const myId = id || 'select-amenities-category' + name + useId()
...
export default SelectOptionPropertyAmenitiesCategory
