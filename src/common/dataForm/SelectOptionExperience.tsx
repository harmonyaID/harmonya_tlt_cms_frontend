...
import useExperienceStore from '@/store/useExperience.store.ts'
...
const SelectOptionExperience = (
    props: SelectOptionGeneralProps & ListDataProps,
) => {
    const ctx = useHookContextForm()

    const { __list, __handlePushDataStore } = useExperienceStore({
        isFormatList: false,
    })
    ...
    const myId = id || 'select-exp' + name + useId()
    ...
    }, [isOnlyChoose, ids, __list.length, props.typeId])
            ...
            {dataList?.length ? (
                <>
                    <p className="fs-12 mb-2 fw-600">
                        Total Experiences : {dataList.length}
                    </p>
                        ...
                        {dataList.map((property, index) => {
                            return (
                                <div
                                    ...
                                    <div className="w-100">{property.name}</div>
                                    <BtnCircleRemove
                                        className="ms-auto"
                                        actions={{
                                            remove: () =>
                                                dataActions.remove(property),
                                        }}
                                    />
                                </div>
                            )
...
export default SelectOptionExperience
