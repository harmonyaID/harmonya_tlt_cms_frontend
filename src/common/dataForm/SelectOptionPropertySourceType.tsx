...
import usePropertySourceType from '@/store/usePropertySourceType.store.ts'
import usePropertyTypeStore from '@/store/usePropertyType.store.ts'

const SelectOptionPropertySourceType = (props: SelectOptionGeneralProps) => {
    const ctx = useHookContextForm()

    const { __list, __isLoading } = usePropertySourceType({
        isFormatList: false,
    })
    ...
    const myId = id || 'select-property-source-type' + name + useId()
...
export default SelectOptionPropertySourceType
