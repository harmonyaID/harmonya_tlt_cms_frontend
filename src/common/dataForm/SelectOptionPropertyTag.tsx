...
import usePropertyTagStore from '@/store/usePropertyTag.store.ts'
...
const SelectOptionPropertyTag = (
    props: SelectOptionGeneralProps & ListDataProps,
) => {
    const ctx = useHookContextForm()

    const { __list, __handlePushDataStore } = usePropertyTagStore({
        isFormatList: false,
    })
    ...
    const myId = id || 'select-property-tags' + name + useId()
    ...
    const _handleAddNewTag = (passData: any = {}) => {
        ...
        apiPropertyTag.add(formRequest).then((res) => {
            if (isSuccess(res)) {
                ...
                <>
                    ...
                    <div
                        className={joinClassNameHelper(
                            'max-h-240px bg-neutral-600 p-3 rounded-2 overflow-auto',
                            { 'mb-3': isMarginBottom },
                        )}>
                        <div className="hstack flex-wrap gap-2">
                            {dataList.map((tag, index) => {
                                return (
                                    <div
                                        className="bg-tint-600 border border-tint-500 text-neutral-100 rounded-2 fw-500 px-2 py-1 hstack gap-2 hover-opacity-75"
                                        key={index}
                                        title="Remove tag"
                                        onClick={() => dataActions.remove(tag)}>
...
export default SelectOptionPropertyTag
