...
import useContactFormInquiryTypeStore from '@/store/useContactFormInquryType.store.ts'

const SelectOptionContactFormInquiryType = (
    props: SelectOptionGeneralProps,
) => {
    const ctx = useHookContextForm()

    const { __list, __isLoading } = useContactFormInquiryTypeStore({
        isFormatList: false,
    })
    ...
    const {
        id = 'select-contact-form-inquiry-type',
        name = '',
        className = '',
        label = '',
    ...
    const myId = id || 'select-contact-form-inquiry-type' + name + useId()
...
export default SelectOptionContactFormInquiryType
