...
import { apiAmenitiesCRUD } from '@/service/api/setting.api.ts'

const SearchPropertyAmenities = (props: any) => {
    // @ts-ignore
    ...
    const idInput = id || 'text-search-amenities-' + props.name + useId()
    ...
    const _handleFilterOptions = (passOptions = []) => {
        ...
        passOptions.forEach((vm) => {
            const index = amenitiesIds.findIndex(
        ...
        apiAmenitiesCRUD
            ...
            .then((resData) => {
                ...
                if (resData) {
                    ...
                    if (!isEmpty(dataAmenities)) {
                        const index: number = newOptions.findIndex(
                            (e) => e[keyName] === dataAmenities[keyName],
                        )
    ...
    useEffect(() => {
        if (!isEmpty(dataAmenities)) {
            !isEmpty(dataAmenities?.name) && isEdit
                ? _handleSearch(dataAmenities?.name)
                : null
        } else {
            _handleSelected({})
        }
    ...
    return (
        <div className={'form-group ' + className}>
            ...
            <AsyncTypeahead
                ...
                isLoading={isLoading}
                minLength={3}
                labelKey={(option: object | any) => `${option.name}`}
                ...
                renderMenuItemChildren={(option: object | any) => {
                    return (
                        <div className="wp-data" key={option[keyName]}>
                            <h5 className="fs-16 fw-500 text-neutral-100 mb-0 mt-1">
                                {option.name || '-'}
                            </h5>
                        </div>
                    )
